...
## 眼睛按钮——复习过程中切换 cloze 模式
上下文树中“其他行”的 cloze 有两种显示模式：**揭示**（蓝色下划线，默认）或**遮挡**（`…`，带 `Context Hide All Test One` 标记的卡片默认如此）。上下文区域**右上角的 👁 眼睛按钮**可为当前这张卡片在两种模式间切换。
...
- 它不会修改知识库：标记仍决定初始模式，切换到下一张卡片时模式自动复位。
...
5. 当前模式不合适时，点击上下文区域右上角的 👁 按钮，即可揭示或隐藏其他行的 cloze 答案。
