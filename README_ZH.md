...
- Context Hide Other Answers（`contextHideAllTestOne`）——作用于**单个 Rem**，而非整棵子树。
  ...
  - 应把它加在**会被剧透的那张 cloze 卡片 Rem（叶子节点）**上，而**不是**加在锚点/父级上。详见下文 [Context Hide Other Answers](#context-hide-other-answers保护会被兄弟节点剧透的-cloze-卡片)。
...
上下文树中“其他行”的 cloze 有两种显示模式：**揭示**（蓝色下划线，默认）或**遮挡**（`…`，带 `Context Hide Other Answers` 标记的卡片默认如此）。上下文区域**右上角的 👁 眼睛按钮**可为当前这张卡片在两种模式间切换。
...
- 普通标记图标 = 为该 Rem **添加** `Context Hide Other Answers`（其他答案保持隐藏）。
...
3. 可选：如果某张 cloze 卡片会被其兄弟节点揭示的答案剧透，就给**这张卡片**添加“Context Hide Other Answers”——运行 **Context: Hide Other Answers for This Rem**（`cfchide`）。详见下文专门章节。
...
## Context Hide Other Answers——保护会被兄弟节点剧透的 cloze 卡片

**功能说明。** 默认情况下，本插件只隐藏你当前正在被测的那个填空。上下文树中其他每一个 cloze——包括兄弟节点以及任何其他 cloze 行——都会显示揭示后的答案（蓝色下划线），让周围的答案充当可见的上下文。

`Context Hide Other Answers` 会为它所应用的那张卡片反转这一行为：当该卡片被复习时，树中**其他所有** cloze 行的答案改为被**遮挡**（显示为 `…`），而不再揭示。注意它与锚点标记的作用范围不同：`Context for Cloze` 加在子树的**根**上并影响全部子代，而本标记加在**单个 Rem** 上，只影响该 Rem 的复习；它也只决定**初始**模式，👁 按钮可随时为当前卡片切换。
...
- **同时标记父级**——父级成为上下文锚点，所选 Rem 获得 `Context Hide Other Answers`。对话框会显示该父级并预先说明后果：锚点会向下级联，此后该父级下的**每一张**卡片都会显示上下文树；同时报告该 Rem 有多少兄弟节点——上下文正由这些兄弟构成，若一个都没有，树会很单薄，更高层的祖先可能才是更合适的锚点。
